...
public class NilColumnType extends AbstractColumnType {

    protected NilColumnType() {
        super("null", -1);
    }

    public ColumnValue createColumnValue(PrepareConfiguration config, int index) {
        return new NilColumnValue(config, index, this);
    }
    ...
    public void filterAndWrite(ColumnValue v, TimeColumnValue filter, RecordWriter with)
            throws PreparePartsException {
        throw new PreparePartsException("nil as time value is unexpected. the row is skipped.");
    }
