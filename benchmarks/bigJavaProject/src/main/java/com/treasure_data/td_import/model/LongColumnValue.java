...
public class LongColumnValue extends AbstractColumnValue {
    private static final Logger LOG = Logger.getLogger(LongColumnValue.class.getName());

    private long v;

    public LongColumnValue(PrepareConfiguration config, int index, ColumnType columnType) {
    ...
    public void parse(String v) throws PreparePartsException {
        ...
        try {
            ...
            String cause = "Column[" + index + "] value cannot be converted to Long type: " + e.getMessage();
            throw new PreparePartsException(cause, e);
        } catch (Exception e) {
            LOG.log(Level.WARNING, String.format(
                    "Cannot parse '%s' to long type", v), e);
